...
public class OVRFaceExpressions : MonoBehaviour, IReadOnlyCollection<float>
{
    ...
    public static float[] currentExpressionWeights;
    public static List<float[]> expressionWeightsAll;
    public static List<string> timestamps;
    public static int elementCountFaceExpressions;
    ...
    private bool StartFaceTracking()
    {
        ...
        expressionWeightsAll = new List<float[]>();
        currentExpressionWeights = new float[63];
        timestamps = new List<string>();
        elementCountFaceExpressions = 0;
        return true;
    }
    ...
    private void OnDisable()
    {
        if (--_trackingInstanceCount == 0)
        {
            ...
            print("elementCountFaceExpressions: " + elementCountFaceExpressions);
            string filePath = DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss-fff") + "_faceexpressions_data.csv";

            StreamWriter outStream = new StreamWriter(filePath);

            string faceExpressionNames = "Timestamp,";
            Array myArray = Enum.GetValues(typeof(FaceExpression));
            int counter = 0;
            foreach (var item in myArray)
            {
                if (item.ToString().Equals("Max"))
                {
                    break;
                }
                faceExpressionNames = faceExpressionNames + item.ToString();
                if (counter != (myArray.Length - 2))
                {
                    faceExpressionNames = faceExpressionNames + ",";
                }
                counter = counter + 1;
            }

            outStream.WriteLine(faceExpressionNames);
            string sb;
            
            for (var i = 0; i < expressionWeightsAll.Count; i++)
            {
                sb = "";
                sb = sb + timestamps[i] + ",";
                for (var j = 0; j < 62; j++)
                {
                    sb = sb + expressionWeightsAll[i][j] + ",";
                }
                sb = sb + expressionWeightsAll[i][62];
    ...
    public float this[FaceExpression expression]
    {
        get
        {
            ...
            print(elementCountFaceExpressions + "....Expression: " + (int)expression + " weight: " + _currentFaceState.ExpressionWeights[(int)expression]);

            if ((int)expression == 0)
            {
                currentExpressionWeights = new float[63];
            }
            currentExpressionWeights[(int)expression] =  _currentFaceState.ExpressionWeights[(int)expression];
            if ((int)expression == 62)
            {
                timestamps.Add(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"));
                expressionWeightsAll.Add(currentExpressionWeights);
                elementCountFaceExpressions = elementCountFaceExpressions + 1;
            }
    ...
    public bool TryGetWeightConfidence(FaceRegionConfidence region, out float weightConfidence)
    {
        ...
        print("Region: " + region + " weight confidence: " + weightConfidence);
        return true;
    }
