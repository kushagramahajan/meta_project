...
def calculate_metrics_prediction_only(predictions, config):
    """Calculate rPPG Metrics (MAE, RMSE, MAPE, Pearson Coef.)."""
    ...
    fields = ['Timestamp (seconds)', 'Heart Rate (PPG Peak Detection)', 'Heart Rate (PPG Fast Fourier Transform)']
    file_counter = 0
    ...
    for index in predictions.keys():
        ...
        file_name = 'output_files/' + data_dirs[file_counter].split('/')[-1].split('.')[0] + '.csv'

        rows = []
        interval_counter = 1

        while (counter < prediction.shape[0]):
            prediction_subset = prediction[counter:counter + interval * fs]
            counter = counter + interval * fs
            ...
            if config.TEST.DATA.PREPROCESS.LABEL_TYPE == "Standardized" or \
                ...
                raise ValueError("Not supported label type in testing!")
            pred_hr_fft = calculate_predictions_per_video(
                prediction_subset, diff_flag=diff_flag_test, fs=config.TEST.DATA.FS, hr_method='FFT')
            pred_hr_peak = calculate_predictions_per_video(
                prediction_subset, diff_flag=diff_flag_test, fs=config.TEST.DATA.FS, hr_method='Peak')
            predict_hr_fft_all.append(pred_hr_fft)
            predict_hr_peak_all.append(pred_hr_peak)

            rows.append([interval * interval_counter, pred_hr_peak, pred_hr_fft])
            interval_counter += 1
        ...
        # print('predict_hr_peak_all: ', predict_hr_peak_all)
        # print('predict_hr_fft_all: ', predict_hr_fft_all)
